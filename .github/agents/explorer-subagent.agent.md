...
Note for this repository: most of `src/` is still an empty scaffold — `api/`, `components/`, `hooks/`,
and `types/` contain only a `.gitkeep`. When a search returns nothing there, that is the real answer.
Report "does not exist yet" rather than proposing the closest unrelated match.
