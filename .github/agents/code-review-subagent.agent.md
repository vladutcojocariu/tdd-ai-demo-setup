...
model: Claude Opus 5
tools: ['read', 'search', 'execute', 'ESLint/*']
agents: []
...
1. Read `.context/current-task.md` if it exists, for the full task context and prior phase notes.
2. Analyze the diff (`git diff`).
3. **Run `npm test`, `npx tsc -b`, and `npm run lint` yourself.** Do not trust the implementer's claim
   that they pass. Independent verification is the point of a separate reviewer. Check the test count —
   `passWithNoTests` makes an empty run exit zero.
4. Read the consolidated patterns in `memory-bank/agent-learnings.md` and flag any finding that matches
   a known recurring issue, explicitly.
