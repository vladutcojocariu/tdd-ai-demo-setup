...
agents:
  ['explorer-subagent', 'planning-subagent', 'architect-review-subagent', 'implement-subagent', 'code-review-subagent']
user-invocable: true
---
...
one exception: removing `{{CONTEXT_FILE}}` at the end of the task.

## Shared context protocol

`{{CONTEXT_FILE}}` lives for the duration of one task. It is ephemeral and gitignored. The planner
creates it after planning; the implementer appends notes to it; every subagent reads it.

```markdown
# Active Task Context

**Task:** <title> **Started:** <timestamp> **Plan:** {{PLANS_DIR}}/<task>-plan.md
...
create `{{CONTEXT_FILE}}` from the template above.
...
red test to write first, which skills to consult, an instruction to read `{{CONTEXT_FILE}}`, and an
instruction to make real edits rather than describing them. If it returns descriptions instead of edits,
re-dispatch with an explicit instruction to use its editing tools.
...
**8. Document.** Dispatch `implement-subagent` to write `{{PLANS_DIR}}/<task>-complete.md` and to
append a retrospective to `{{MEMORY_DIR}}/agent-learnings.md` in the format below, pruning per the
rolling-window rule. Then remove `{{CONTEXT_FILE}}` and present the summary.
