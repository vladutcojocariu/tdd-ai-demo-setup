...
terminal. The only files you create are the plan in `{{PLANS_DIR}}` and, when the conductor asks for
it, the task context file `{{CONTEXT_FILE}}`. Creating any other file is out of scope.
...
2. **Read `{{MEMORY_DIR}}/agent-learnings.md`**, the consolidated patterns section, and
   `{{MEMORY_DIR}}/lessons.md`. Flag any risk in your plan that matches a known past failure.
...
6. **Write the plan** to `{{PLANS_DIR}}/<task-name>-plan.md`.
7. **If asked, create `{{CONTEXT_FILE}}`** using the template in your dispatch prompt.

## TDD mandate

Every phase must name the failing test to write first, with a concrete test name and file path. A phase
that cannot name its red test is not ready to implement. If the change is user-visible and end-to-end
testing is in use, include a dedicated end-to-end phase.
