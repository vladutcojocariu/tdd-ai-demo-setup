...
description: >
  ...
  when an architectural decision is needed, or when the user says build, create, implement, or add
  something non-trivial. Skip only for trivial changes or an already-approved plan.
---
...
1. **Explore context.** Read the relevant existing code and `{{MEMORY_DIR}}/lessons.md` for prior decisions.
