...
description: >
  ...
  agent", "audit skill", "skill routing", "improve skill", "why didn't the skill load". Do NOT trigger
  for merely using a skill.
---
...
6. **Integrate.** Add it to the routing table in `.github/copilot-instructions.md`, and to an
   instructions file if it needs auto-loading. Note anything novel in `memory-bank/lessons.md`.
...
**Good:** `description: Enforce unit test requirements for all source changes using Vitest with
red-green-refactor. Trigger whenever code is written or modified, or the user mentions test, TDD, or
coverage. Do NOT trigger for pure documentation or configuration changes.`

Be slightly pushy. Models under-trigger far more often than they over-trigger.

## This project's conventions

Generated files must keep matching the stack. When you add or edit one:

- Commands are `npm test`, `npx vitest run <file>`, `npx tsc -b`, `npm run lint`, `npm run build`.
  Never another package manager.
- Skills live in `.github/skills/<name>/SKILL.md`; agents in `.github/agents/<name>.agent.md`.
- The source of truth for every command and path is `memory-bank/stack-profile.md`. Update it in the
  same change, or the next bootstrap run will regenerate against stale values.
- End-to-end testing is inactive. Do not reference `e2e-*` skills or the `e2e-journey` agent — they do
  not exist. To add them, install a runner, write a spec, and re-run the `ai-tests-bootstrap` skill.
- Run `npx prettier --write` on any Markdown you generate here; these files are covered by the
  formatter.
...
- **List MCP tools (`<server>/*`) only for servers configured in `.vscode/mcp.json`.** Unavailable
  tools are silently ignored, so a typo fails without an error. The ESLint server is configured under
  the key `ESLint`, so its tool pattern is `ESLint/*` — the casing matters.
- **Every agent pins a `model`.** The team agreed on three tiers: `Gemini 3.8 Flash` for
  `explorer-subagent`, `Claude Sonnet 5` for `conductor`, `implement-subagent`, and `solo-dev`, and
  `Claude Opus 5` for `planning-subagent`, `architect-review-subagent`, and `code-review-subagent`. A
  new agent picks the tier that matches its job. The name must match the chat model picker exactly —
  an unrecognised value is ignored without an error, and the agent quietly runs on whatever is
  selected instead.
